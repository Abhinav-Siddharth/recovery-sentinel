...
def choose_failure(method: str, degraded: bool) -> str:
    if degraded:
        # The degraded ROUTE_B/UPI incident is mostly transient
        # technical/network failures, but includes a smaller set
        # of customer-action failures. This allows Recovery Sentinel
        # to demonstrate PAYMENT_LINK recovery where blind retry
...
def generate():
    ...
    for i in range(1, 501):
        ...
        if degradation:
            # Keep the systemic incident centered on
            # ROUTE_B + UPI so the detector remains stable.
        ...
        if failed:
            failure_code = choose_failure(
                method,
                degradation,
            )

            recoverable, ground_truth_action = (
                action_for_failure(
                    failure_code,
                    amount,
                )
    ...
    print(
        "Degraded ROUTE_B/UPI "
        f"failure rate: "
        f"{degraded_failure_rate:.2%}"
    )

    print(
        f"Degradation failures: "
        f"{len(affected)}"
    )

    print(
        f"Revenue at risk: "
        f"₹{affected['amount'].sum():,.0f}"
    )
