...
type DashboardState = {
  ...
  applyPayload: (data: DashboardPayload) => void
}
...
export function useDashboardData(): DashboardState {
  ...
  const applyPayload = useCallback((data: DashboardPayload) => {
  ...
  const applyIncident = useCallback((next: IncidentResponse) => {
    setIncident(next)
  }, [])

  const applyMetrics = useCallback((next: MetricsResponse) => {
    setMetrics(next)
  }, [])
